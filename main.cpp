...
{
  if(argc<=1) exit(0);
  int nb_sides=atoi(argv[1]);  

  vector<long> nbt;

  for(int n=3; n<=nb_sides; n++)
    {
      cout << "n : " << n << endl;
      decomposition polygon(n);
      vector<decomposition> ld; ld.push_back(polygon);
      tree<decomposition> t;
      tree<decomposition>::iterator t_iter;
      
      t_iter=t.insert(t.begin(), polygon);
      make_dec_tree(t, t_iter, ld);
      
      vector<decomposition> vt=select_triangulation(ld);
      nbt.push_back(vt.size());
